...
df = pd.read_json(CRE_output_fmt).loc["2020-01-12":"2020-02-18"]
...
logging.info("Shallow cloud days only statistics (high clouds filtered)")
...
for p, (product, product_label) in enumerate(
    ...
    axs[p].text(
        ...
        + f"={slope:.2f}"
        + r"$\cdot \mathrm{CRE'}_{\mathrm{ICON}}$"
        + f"+{intercept:.2f}",
    )
