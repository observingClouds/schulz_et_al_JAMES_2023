...
if __name__ == "__main__":  # noqa: C901
    ...
    for setup in ["control"]:  # ['highCCN', 'control', 'both']:
        fig, axs = plt.subplots(
            1, n_subplots, sharey=False, sharex=False, figsize=(3, 2), dpi=300
        )

        for p, ds_sel in enumerate(
            [
                ds_out.sel(
                    time=~ds_out.control_CF_ICON_DOM02.isnull().any(dim="height")
                ),
                ds_out.sel(
                    time=~ds_out.control_CF_ICON_DOM02.isnull().any(dim="height")
                ).sel(time=slice("2020-02-01", "2020-02-07")),
            ]
        ):
            ...
            logging.info(
                f'OBS: @2km:{mean.CF_kaband.sel(range=2000, method="nearest").item(0)}'
            )
            mean_DOM1 = mean.control_CF_ICON_DOM01.sel(
                height=2000, method="nearest"
            ).item(0)
            logging.info("Control DOM01:" + f" @2km:{mean_DOM1}")
            mean_DOM2 = mean.control_CF_ICON_DOM02.sel(
                height=2000, method="nearest"
            ).item(0)
            logging.info("Control DOM02:" + f" @2km:{mean_DOM2}")
            mean_DOM3 = mean.control_CF_ICON_DOM03.sel(
                height=2000, method="nearest"
            ).item(0)
            logging.info("Control DOM03:" + f" @2km:{mean_DOM3}")
                ...
                if setup in ["control", "both"]:
                    ...
                    if p == 1:
                        axs[p].fill_betweenx(
                            ...
                            np.max(
                                [
                                    np.zeros(len(mean.control_CF_ICON_DOM03)),
                                    mean.control_CF_ICON_DOM03
                                    - sem(
                                        resample(
                                            ds_sel.dropna(
                                                dim="time"
                                            ).control_CF_ICON_DOM03,
                                            n_samples=bootstrap_samples,
                                        ),
                                        axis=0,
                                    ),
                            ...
                            + sem(
                                resample(
                                    ds_sel.dropna("time").control_CF_ICON_DOM03,
                                    n_samples=bootstrap_samples,
                                ),
                                axis=0,
                            ),
        ...
        fig.subplots_adjust(bottom=0.3, wspace=0.93)
        ...
        out_fn = "FIG_CF_profile_{res}-variability_totals_{setup}_err-{err}.pdf".format(
